...
class Console extends CI_Controller
{
    ...
    public function writeStats() {
        ...
        foreach ($this->msg as $data) {
            $this->db->insert('offers', [
                ...
                'year'  =>  $data['car']['year'],
                'link'  =>  $data['car']['href'],
                'price_ad'  =>  $data['car']['price'],
                'phone'  =>  $data['reciver'],
                'price_offer'  =>  $data['offer'],
                'txt'  =>  $data['text'],
                'geo'  =>  $data['geo'],
                'user' => $this->user['id'],
                'kpp_type' => $data['car']['kpp_type']
            ]);
        }
    }
    ...
    private function loadCars()
    {
        if (is_null($this->user['parsed_file'])) {
            ...
            $this->db->update('users', [
                'parsed_file' => serialize($this->car_to_sms)
            ]);
    ...
    public function sendMsg($msg, $phone)
    {
        ...
        return;
        $this->getSms()->execCommad('sendSMS', array(
            'sender' => 'SMS',
            'sms_lifetime' => 0,
            'text' => $msg,
            'phone' => $phone,
...
//        die();
    ...
    private function checkMail()
    {
        ...
        foreach ($mailsIds as $c => $mailId) {
            ...
            $mail_id = $this->db->insert('mails', [
                'user_id' => $this->user['id'],
                'mail_id' => $mailId,
                'theme' => $mail->subject,
                'data' => $mail->textHtml . $mail->textPlain,
                'date' => $mail->date,
            ]);
            ...
            if ($this->inarr($mail->fromAddress, $this->user['parser'])) {
                ...
                if ($mail->subject == 's1')
                    $data = $this->processFromCsv($mail->textPlain);
                else
...
// If the files uses an encoding other than UTF-8 or ASCII, then tell the reader
        ...
        for ($i = 1; $i < count($this->user['report']); $i++)
            $email->AddCC($this->user['report'][$i]);
    ...
    public function checkBalance()
    {
        ...
        $status = [$this->user['min_balance_ms1_trigger'],
            $this->user['min_balance_ms2_trigger'],
            $this->user['min_balance_ms3_trigger']
        ];
        if ($this->user['min_balance_milestones'][0] > $balance && ($this->user['min_balance_milestones'][1] < $balance) && $status[0]) {
            $this->minBalance($balance, $this->user['min_balance_milestones']['phone']);
            $status[0] = false;
        ...
        if ($balance > $this->user['min_balance_milestones'][2])
            $status[2] = true;
        if ($balance > $this->user['min_balance_milestones'][1])
            $status[1] = true;
        if ($balance > $this->user['min_balance_milestones'][0])
            $status[0] = true;

        $this->db->where('id', $this->user['id']);
        $this->db->update('users', [
            'min_balance_ms1_trigger' => $status[0],
            'min_balance_ms2_trigger' => $status[1],
            'min_balance_ms3_trigger' => $status[2]
        ]);
        return true;
    }
    ...
    public function END_ACTIVITY()
    {
        ...
        $out = ob_get_contents();
        $this->db->insert('logs', ['log'=>$out, 'user_id'=>$this->user['id']]);
        ob_flush();
    }
    ...
    {
        $this->load->library('ApiSms', array(
            'api_private' => $this->api_private,
            'api_pub' => $this->api_public,
            'url_gateway' => $this->URL_GAREWAY
        ));
        return $this->apisms;
    }
    ...
    private function report()
    {
        ...
        if ($m == 30 || $m == 0) {
            foreach ($this->user['report_time'] as $r)
                if ("$h:$m" == $r)
                    $this->sendReport("24 HOUR", str_replace("%DATE%", \date('d/m/Y'), $this->user['mail_theme_1']));
            foreach ($this->user['report_time_single'] as $r)
                if ("$h:$m" == $r)
                    $this->sendReport("1 YEAR", $this->user['mail_theme_2']);
        }
    }
